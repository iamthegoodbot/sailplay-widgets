(function (angular, sp, jQuery) {
    ...
    angular.module('lgb.directives.badges', ['lgb.services.statuses', 'lgb.services.actions'])

        .directive('badgesCmp', ['actionService', 'statusService', function (actionService, statusService) {
            return {
                restrict: 'E',
                replace: true,
                ...
                link: function (scope) {
                    ...
                    function update() {
                        actionService.loadList().then(function () {
                            statusService.loadList().then(function (data) {
                                scope.badges = data.multilevel_badges[0];
                    ...
                    scope.showBadgeImage = function (badge) {
                        if (!badge) return;
                        var imgs = statusService.getBadgeImage(badge.id);
    ...
    document.createElement('badges-cmp');
    var elems = document.querySelectorAll('badges-cmp');
    for (var i = 0; i < elems.length; i += 1) {
        angular.bootstrap(elems[i], ['lgb.directives.badges']);
    }

                            ...
                            badgeService.loadList().then(function (data) {
                                ...
                                if (scope.statuses.length) {
                                    ...
                                    for (var i = 0, len = scope.statuses.length; i < len; i++) {
                                        if (scope.statuses[i].is_received && ((scope.statuses[i + 1] && !scope.statuses[i + 1].is_received) || !scope.statuses[i + 1])) {
                                            ...
                                            scope.points_to_next_status = scope.next_status ? (scope.getSumForStatuses()[i + 1] - scope.user.purchases.sum) : scope.getSumForStatuses()[0];
                                            if(scope.points_to_next_status < 0) {
                                                scope.points_to_next_status = 0;
                                            }
                                            break;
                                        }
                                    }
                                    giftService.loadList().then(function (data) {
                                        ...
                                        var p = Math.round(( scope.user.user_points.confirmed / max.points) * 100);
                                        scope.persents_to_next_status = p > 100 ? 100 : p;
                                        scope.$digest();
                                    });
                                }
                                scope.$digest();

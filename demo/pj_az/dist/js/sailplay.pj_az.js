...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/html/main.html',
    '<div class="sp_wrapper" data-ng-show="loaded"><div class="bns_inner_block clearfix"><div class="bns_top" data-ng-if="!auth" style="background-image: url({{ headerBg }})"><div class="bns_top_info"><div class="bns_top_slide"><img src="https://sailplays3.cdnvideo.ru/media/assets/assetfile/6c38af1c9d8e9d24bd226201c02e0459.png"> <span class="bns_slide_text">У нас есть потрясающая бонусная программа для вас, благодаря которой вы можете<br>получать баллы за покупки товаров на нашем сайте. За эти баллы вы сможете получать<br>подарки из нашего ассортимента!</span><div class="bns_top_slide_linck"><a data-ng-href="{{ config.data && config.data.login_link }}">Войти</a></div></div></div></div><div data-ng-if="auth"><div class="bns_top" style="background-image: url({{ $parent.headerBg }})"><div class="bns_top_info clearfix" data-sailplay-profile=""><div class="bns_top_info_left" data-ng-if="!showHistory"><div class="bns_ava_block"><img data-ng-src="{{ user().user.pic }}" class="bns_ava"><div class="bns_ava_block_right"><span class="bns_name" data-ng-bind="user().user.first_name || user().user.last_name ? user().user.first_name + \' \' + user().user.last_name : \'Имя не указано\'"></span> <a href="#" data-ng-click="$event.preventDefault();show_history();">История начислений</a></div></div><div class="bns_top_info_stat"><div class="bns_ne_pod_bal_block"><div class="bns_ne_pod_bal" data-ng-bind="user().user_points.unconfirmed | number"></div><span class="bns_ne_pod_bal_name">Неподтвержденные баллы</span> <a href="javascript:void(0);">Что это? <span class="bns_tile">Ваши неподтвержденные баллы, которые станут подтверждены после факта оплаты заказа</span></a></div><div class="bns_stat_now"><img data-ng-src="{{ user().last_badge && user().last_badge.pic }}"> <span class="bns_stat_name" data-ng-bind="user().last_badge && user().last_badge.name ? \'Вы — \' + user().last_badge.name : \'Нет статуса\'"></span> <a data-ng-href="{{ config.data && config.data.status_link }}" target="_blank">Список статусов</a></div></div></div><div class="bns_top_popup bns_top_popup_hist" data-sailplay-history="" data-ng-if="showHistory"><a href="#" class="close" data-ng-click="$event.preventDefault();clear_show(\'showHistory\');"></a><div class="bns_top_popup_hist"><h2>История начислений</h2><div class="bns_top_popup_hist_item" data-dir-paginate="item in history() | itemsPerPage:5" data-pagination-id="history_pages"><span class="date" data-ng-bind="item.action_date | date:\'dd.MM.yyyy\'"></span> <span class="name" data-ng-bind="item | history_item"></span> <span class="bal" data-ng-class="{bal_min: item.points_delta < 0, bal_plus: item.points_delta >= 0}" data-ng-bind="item.points_delta && ((item.points_delta > 0 ? \'+\' : \'\') + (item.points_delta | number)) + \' \' + (item.points_delta |sailplay_pluralize:\'балл,балла,баллов\')"></span></div><dir-pagination-controls data-max-size="5" data-pagination-id="history_pages" data-template-url="/html/ui/ui.pagination.controls.html" data-auto-hide="true"></dir-pagination-controls></div></div><div class="bns_top_info_right" data-ng-show="!showHistory"><sailplay-pizzameter data-model="user().user_points.confirmed" style="width:100%; height: 100%;"></sailplay-pizzameter></div></div></div><div class="bns_tab"><a href="#" data-ng-repeat="tab in tabs track by $index" data-ng-click="$event.preventDefault();$parent.$parent.activeTab = tab.alias;" data-ng-class="{act: tab.alias == $parent.activeTab}" data-ng-bind="tab.name"></a></div><div data-ng-switch="$parent.activeTab"><div class="bns_tab_block" id="gift" data-ng-switch-when="gifts" data-sailplay-gifts=""><div class="bns_gift_item" data-ng-repeat="gift in gifts()"><img data-ng-src="{{ gift.thumbs.url_250x250 | sailplay_pic }}"><div class="bns_gift_item_bottom"><span class="bns_gift_name" data-ng-bind="gift.name"></span> <span class="bns_gift_desc" data-ng-bind="gift.descr"></span></div><a href="#" data-ng-class="{act: user().user_points.confirmed >= gift.points}" data-ng-click="$event.preventDefault();get(gift);">{{ (gift.points | number) + \' \' + (gift.points | sailplay_pluralize:\'балл,балла,баллов\') }} <span class="bns_til" data-ng-if="user().user_points.confirmed < gift.points" data-ng-bind="\'Нужно накопить еще \' + ((gift.points - user().user_points.confirmed) | number) + \' \' + ((gift.points - user().user_points.confirmed) | sailplay_pluralize:\'балл,балла,баллов\')"></span></a></div></div><div class="bns_tab_block" id="qust" data-ng-switch-when="actions" data-sailplay-actions=""><div class="bns_qust_item" data-ng-repeat="action in actions"><div class="bns_qust_left"><img data-ng-src="{{ action_data(action).image }}"> <img data-ng-src="{{ action_data(action).image_h }}" class="bns_qust_left_h"></div><div class="bns_qust_right"><span class="bns_qust_right_name" data-ng-bind-html="action_data(action).name | to_trusted"></span> <span class="bns_qust_right_bal" data-ng-bind="(action.points | number) + \' \' + (action.points | sailplay_pluralize:\'балл,балла,баллов\')"></span> <a href="#" data-sailplay-action="" data-action="action" data-text="Выполнить" data-styles="{{ action_styles(action_data(action)) }}">Выполнить</a></div></div></div><div class="bns_tab_block" id="achiv" data-ng-switch-when="badges" data-sailplay-badges=""><div class="bns_achiv_line" data-ng-repeat="line in badges().multilevel_badges"><div class="bns_achiv_item" data-ng-repeat="badge in line" data-ng-class="{act: badge.is_received}"><img data-ng-src="{{ (badge.is_received ? badge.thumbs.url_250x250 : badge.thumbs.url_gs) | sailplay_pic }}"> <span>{{ badge.name }} <strong data-ng-show="badge.points" data-ng-bind="(badge.points | number) + \' \' + (badge.points |sailplay_pluralize:\'балл,балла,баллов\')"></strong></span></div></div></div></div></div></div><notify-popup></notify-popup></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/html/ui/ui.pagination.controls.html',
    '<div class="bns_hist_page" data-ng-if="1 < pages.length || !autoHide"><span>Страницы:</span> <a href="#" data-ng-repeat="pageNumber in pages track by tracker(pageNumber, $index)" data-ng-class="{ \'act\' : pagination.current == pageNumber }" data-ng-click="setCurrent(pageNumber);$event.preventDefault();" data-ng-bind="pageNumber"></a></div>');
}]);
})();

        ...
        JSONP.get((params.domain || '//sailplay.ru') + '/js-api/' + params.partner_id + '/config/', { lang: params.lang || 'ru', dep_id: (params.dep_id || '') }, function (response) {
            if (response && response.status == 'ok') {
                ...
                function onActionMessage(messageEvent) {
                    var data= {};
                    var mesDomain = messageEvent.origin.slice(0,4) == 'http' ? messageEvent.origin.slice(5) : messageEvent.origin.slice(0,5) == 'https' ? messageEvent.origin.slice(6) : messageEvent.origin;
                    if(mesDomain == _config.DOMAIN) {
                        try {
                            data = JSON.parse(messageEvent.data);
                        }
                        catch (e){
...
//        cookies.eraseCookie('sp_auth_hash');
    ...
    sp.on('tags.add', function (data) {
        ...
        if (data && _config.auth_hash || (data && !Array.isArray(data) && (data.identify.email || data.identify.phone || data.identify.origin_user_id))) {
            var obj = {
                tags: Array.isArray(data) ? data.join(',') : data.tags.join(',')
            };
            if(!Array.isArray(data) && (data.identify.email || data.identify.phone || data.identify.origin_user_id)){
                obj = extend(obj, data.identify)
            } else {
                obj.auth_hash = _config.auth_hash;
            }
            JSONP.get(_config.DOMAIN + _config.urls.tags.add, obj, function (res) {
                if (res.status == 'ok') {
